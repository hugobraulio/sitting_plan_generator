...
var regex_CALM_localhost = /http:\/\/localhost:3000\/(\w|-)+\/courses\/\d+\/course_applications*/;
...
function disableBrowserAction(){
	...
	chrome.browserAction.setPopup({popup: "html/popup_disabled.html"});
...
function enableBrowserAction(){
	...
	chrome.browserAction.setPopup({popup: "html/popup_enabled.html"});
    //chrome.tabs.executeScript(null, {file: "contentscript.js"});
}

function updateState(tab){
	var url = tab.url;
	var enabled =  regex_CALM_test.test(url) || regex_CALM_production.test(url) || regex_CALM_localhost.test(url); //regExCALM_desarrollo.test(url) ||
	if(enabled){ //URL of a CALM course!
		...
		url_POST_seat_map = url_DDBS_course + "/assign_generated_hall_positions"; //Upload sitting map to CALM
