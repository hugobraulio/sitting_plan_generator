...
Layout.prototype.getNumberOfRows = function(gender){
    return this.hall.getNumberOfRows(gender);
}
...
* the females have hall_position==""). So in this method, we just use .getHallLabels() as a bag of labels. Order does not matter.
...
*that matters is their 'hall_position' value, not their phisical arrangement on the screen.
*/
Layout.prototype.getContent = function(){ 
	...
	for(var i=0; i < v.length; i++){ 
		var label = v[i].hall_position; //get students hall coordinate (if any)
		...
		else{  //if the student already had a hall_position..
...
* Recorrer todas las hall_position[gender] hasta encontrar la "B2" e introducir el numero de columnas en la posicion A1
...
/** For TESTING porpouses. Sets all 'hall_position'  to "", and columns and rows to 6 and 8
*/
Layout.prototype.reset_all_positions = function(){
	...
	var json = { "hall_positions" : {},
                 "hall_config": { "male": {"number_of_rows": "8", "number_of_columns": "6"},
                                  "female": {"number_of_rows": "8", "number_of_columns": "6"}}};
	for(var i = 0; i < v.length; i++){
...
* sent data should look like:
				{ "hall_positions" : {"234" : "A1", "128": "A2" , "561" : "F4", "104" : "F5" },
				   "male": {"number_of_rows": 9, "number_of_columns": 5},
				   "female": {"number_of_rows": 6, "number_of_columns": 6}
				}
...
Layout.prototype.save_to_CALM = function(alternative_content) {
    ...
    var hallConfig = { "male" : {"number_of_rows": this.getNumberOfRows("male"), "number_of_columns" : this.getNumberOfColumns("male")},
        "female" : {"number_of_rows": this.getNumberOfRows("female"), "number_of_columns" : this.getNumberOfColumns("female")}
    };
	var savedContent = alternative_content|| saveContent(hallConfig, this.idTable, "json");
			/*
...
function saveContent(hallConfig, tbl, type) {
	...
	if (tbl !== undefined && typeof(tbl) === 'object' && tbl.nodeName === 'TABLE') {
        ...
        var male_rows = hallConfig["male"]["number_of_rows"];
        var male_cols = hallConfig["male"]["number_of_columns"];
        var female_rows = hallConfig["female"]["number_of_rows"];
        var female_cols = hallConfig["female"]["number_of_columns"];
		if (type === 'json' && count > 0) {
			query = { "hall_positions": JSONobj,
                      "male": {"number_of_rows": male_rows, "number_of_columns": male_cols},
                      "female": {"number_of_rows": female_rows, "number_of_columns": female_cols}
            };
		}
		else {
